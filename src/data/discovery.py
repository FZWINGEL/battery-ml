...
def discover_experiment_files(
    base_path: Path, experiment_id: int
) -> Dict[str, List[Path]]:
    ...
    cycle_dir = (
        paths.expt_path / "Summary Data" / "Ageing Sets Summary" / "Summary per Cycle"
    )
    if cycle_dir.exists():
    ...
    # Find set summaries
    set_dir = (
        paths.expt_path / "Summary Data" / "Ageing Sets Summary" / "Summary per Set"
    )
    if set_dir.exists():
...
def parse_filename_metadata(filename: str) -> Dict[str, Any]:
    ...
    if (
        curve_match
        and "deg" not in filename[max(0, filename.find(curve_match.group(0)) - 3) :]
    ):
