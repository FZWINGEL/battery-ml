...
class ExperimentPaths:
    ...
    def __post_init__(self):
        ...
        if self.experiment_id not in self.EXPERIMENT_FOLDERS:
            raise ValueError(
                f"Invalid experiment_id: {self.experiment_id}. Must be 1-5."
            )
    ...
    def performance_summary(self, cell_id: str, temp_C: int) -> Path:
        ...
        filename = (
            f"Expt {exp_label} - cell {cell_id} ({temp_C}degC) - Processed Data.csv"
        )
    ...
    def list_available_rpts(self, cell_id: str, curve_type: str = "0.1C") -> List[int]:
        ...
        rpts = []
        for f in curve_dir.glob("*RPT*discharge*.csv"):
