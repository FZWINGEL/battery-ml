...
class SummaryDataLoader:
    ...
    def load_all_cells(
        self, cells: List[str], temp_map: Dict[int, List[str]]
    ) -> pd.DataFrame:
            ...
            for cell_id in temp_cells:
                if cell_id in cells:
                    try:
                        ...
                        dfs.append(df)
                        logger.info(
                            f"Loaded cell {cell_id} at {temp_C}°C: {len(df)} samples"
                        )
...
class TimeseriesDataLoader:
    ...
    def load_all_curves(
        self, cell_id: str, curve_type: str = "0.1C", direction: str = "discharge"
        ...
        for rpt in rpts:
            try:
                curves[rpt] = self.load_voltage_curve(
                    cell_id, rpt, curve_type, direction
                )
