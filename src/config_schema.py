...
class PipelineConfig(BaseModel):
    ...
    name: Literal["summary_set", "summary_cycle", "ica_peaks", "latent_ode_seq"] = Field(
        description="Pipeline name"
    )
    
    # ICA-specific params
    sg_window: int = Field(default=51, ge=5, le=101, description="Savitzky-Golay window")
    sg_order: int = Field(default=3, ge=1, le=5, description="Savitzky-Golay polynomial order")
    num_peaks: int = Field(default=3, ge=1, le=10, description="Number of peaks to extract")
    voltage_range: Tuple[float, float] = Field(
        default=(3.0, 4.2), description="Voltage range for ICA"
    )
    
    # Sequence params
    seq_length: Optional[int] = Field(default=None, ge=1, description="Max sequence length")
    ...
    include_arrhenius: bool = Field(default=True, description="Include Arrhenius features")
    arrhenius_Ea: float = Field(default=50000.0, gt=0, description="Activation energy (J/mol)")
...
class ModelConfig(BaseModel):
    ...
    hidden_dims: List[int] = Field(default=[64, 32], description="Hidden dimensions for MLP")
    ...
    use_adjoint: bool = Field(default=True, description="Use adjoint method for gradients")
...
class SplitConfig(BaseModel):
    ...
    train_temps: List[int] = Field(default=[10, 40], description="Training temperatures")
    ...
    val_fraction: float = Field(default=0.2, ge=0, le=1, description="Validation fraction")
...
class TrainingConfig(BaseModel):
    ...
    early_stopping_patience: int = Field(default=20, ge=1, description="Early stopping patience")
    gradient_clip: float = Field(default=1.0, gt=0, description="Gradient clipping value")
    use_amp: bool = Field(default=True, description="Use automatic mixed precision")
    scheduler_T0: int = Field(default=50, ge=1, description="Scheduler T0 for cosine annealing")
...
class LossConfig(BaseModel):
    ...
    monotonicity_weight: float = Field(
        default=0.0, ge=0, description="Monotonicity regularization weight (physics_informed)"
    )
    smoothness_weight: float = Field(
        default=0.0, ge=0, description="Smoothness regularization weight (physics_informed)"
    )
    
    # Huber params
    delta: float = Field(
        default=1.0, gt=0, description="Huber delta threshold (huber)"
    )
