...
class ExperimentRunner:
    ...
    def load_data(self) -> Any:
        ...
        try:
            ...
            data = DataLoaderRegistry.load_data(
                ...
                temp_map={
                    int(k): list(v) for k, v in self.config.data.temp_map.items()
                },
            )
            ...
            if self.data_format == "summary":
                df = data["df"]
                print(
                    f"  ✓ Loaded {len(df)} samples from {df['cell_id'].nunique()} cells"
                )
    ...
    def setup_pipeline(self) -> None:
        ...
        self.pipeline = PipelineRegistry.get(
            self.config.pipeline.name, **pipeline_params
        )
    ...
    def split_data(self, samples: List) -> Tuple[List, List]:
        ...
        if self.config.split.strategy == "temperature_holdout":
            ...
            print(
                f"  ✓ Temperature holdout: {self.config.split.train_temps} -> {self.config.split.val_temps}"
            )

        elif self.config.split.strategy == "loco":
            train_samples, val_samples = leave_one_cell_out(
                samples, test_cell=self.config.split.test_cell
            )
    ...
    def setup_tracker(self) -> None:
        ...
        if self.config.tracking.backend == "dual":
            ...
            self.tracker = LocalTracker(base_dir=str(project_root / "artifacts/runs"))

        print(f"  ✓ Tracker: {self.config.tracking.backend}")

    def train_and_evaluate(
        self, train_samples: List, val_samples: List
    ) -> Dict[str, float]:
        ...
        if self.config.model.name == "lgbm":
            ...
            if self.config.loss.name == "huber":
                ...
                loss_config["monotonicity_weight"] = (
                    self.config.loss.monotonicity_weight
                )
            ...
            trainer = Trainer(
                model=self.model, config=training_config, loss_config=loss_config
            )
    ...
    def log_results(
        self, metrics: Dict[str, float], train_samples: List, val_samples: List
    ) -> str:
    ...
    def run(self) -> Dict[str, float]:
        ...
        if input_dim is None:
            raise ValueError(
                "No samples created - cannot determine input dimension for model"
            )
