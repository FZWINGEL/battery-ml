...
class ODEFunc(nn.Module):
    ...
    def __init__(
        self, hidden_dim: int, augment_dim: int = 20, ode_hidden_dim: int = 128
    ):
...
class ACLAModel(BaseModel):
    ...
    def __init__(
        ...
        if not HAS_TORCHDIFFEQ:
            raise ImportError(
                "torchdiffeq required. Install with: pip install torchdiffeq"
            )
        ...
        self.encoder = CNNLSTMEncoder(
            input_dim=input_dim, cnn_filters=cnn_filters, lstm_hidden=hidden_dim
        )
    ...
    def explain(
        ...
        return {
            "attention_weights": (
                self._last_attn_weights.cpu().numpy()
                if self._last_attn_weights is not None
                else None
            ),
            "trajectory": (
                self._trajectory.cpu().numpy() if self._trajectory is not None else None
            ),
