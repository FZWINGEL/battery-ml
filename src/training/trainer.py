...
class Trainer:
    ...
    def __init__(
        ...
        # AMP
        self.use_amp = config.get("use_amp", True) and device == "cuda" and HAS_AMP
        if self.use_amp:
    ...
    def _samples_to_loader(
        self, samples: List[Sample], batch_size: int, shuffle: bool
    ) -> DataLoader:
    ...
    def fit(
        ...
        for epoch in range(epochs):
            ...
            if self.verbose:
                ...
                logger.info(
                    f"Epoch {epoch+1}/{epochs} - Train: {train_loss:.5f}, Val: {val_loss:.5f}, LR: {current_lr:.2e}"
                )
