...
def evaluate_by_group(
    samples: List[Sample], predictions: np.ndarray, group_key: str = "temperature_C"
    ...
    for i, sample in enumerate(samples):
        ...
        y_true = (
            sample.y.numpy().item() if hasattr(sample.y, "numpy") else float(sample.y)
        )
...
def print_grouped_metrics(
    grouped_metrics: Dict[str, Dict[str, float]], group_name: str = "Group"
) -> None:
    ...
    for group, metrics in sorted(grouped_metrics.items()):
        print(f"\n{group_name} = {group}:")
        print(
            f"  RMSE: {metrics['rmse']:.5f}, MAE: {metrics['mae']:.5f}, R²: {metrics['r2']:.4f}"
        )
