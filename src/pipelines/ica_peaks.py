...
class ICAPeaksPipeline(BasePipeline):
    ...
    def compute_ica(
        self, voltage: np.ndarray, capacity: np.ndarray
    ...
    def extract_peak_features(self, V: np.ndarray, dQdV: np.ndarray) -> np.ndarray:
        ...
        # Find peaks
        peaks, properties = find_peaks(dQdV, height=0.01, distance=200, prominence=0.01)

        # Get peak widths
        if len(peaks) > 0:
            try:
                widths, width_heights, left_ips, right_ips = peak_widths(
                    dQdV, peaks, rel_height=0.5
                )
            except Exception:
            ...
            if i < len(peaks):
                ...
                features.extend(
                    [
                        ...
                        (
                            dQdV[peak_idx] * widths[i] * dV if i < len(widths) else 0
                        ),  # approx area
                    ]
                )
            else:
