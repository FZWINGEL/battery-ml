...
def sample_summary_df():
    ...
    df = pd.DataFrame({
        ...
        'Cell Capacity [mA h]': 5000 - np.linspace(0, 500, n_samples) + np.random.randn(n_samples) * 20,
        'cell_id': ['A'] * 10 + ['B'] * 10,
...
def sample_voltage_curve():
    ...
    voltage = 4.2 - 0.3 * capacity + 0.05 * np.sin(capacity * 2) + np.random.randn(n_points) * 0.01
...
def sample_curves_with_meta():
    ...
    targets = {('A', 0): 4.8, ('A', 1): 4.7, ('A', 2): 4.6,
               ('B', 0): 4.9, ('B', 1): 4.8, ('B', 2): 4.7}
